...
import { IconPhoto, IconPlus, IconTrash, IconClock } from "@tabler/icons-react";
...
function ProjectCard({ project, onOpen, onDelete }) {
    const lastImage = project.generations[0]?.images[0];
    ...
    return (
        ...
        >
            {/* Thumbnail */}
            <Box style={{ width: "100%", aspectRatio: "16/9", background: "var(--gray-3)", overflow: "hidden" }}>
                {lastImage ? (
                    <img
                        src={lastImage.url}
                        alt={project.name}
                    ...
                    <Flex align="center" justify="center" style={{ width: "100%", height: "100%" }}>
                        <IconPhoto size={32} style={{ color: "var(--gray-7)" }} />
                    </Flex>
                )}
            </Box>
...
export default function ImageListPage() {
    ...
    const { projects, createProject, deleteProject } = useImageStore();
    const [dialogOpen, setDialogOpen] = useState(false);
    const [newName, setNewName] = useState("");

    const handleCreate = () => {
        const name = newName.trim() || `Image Project ${new Date().toLocaleDateString()}`;
                ...
                <Flex direction="column" gap="2">
                    <Flex align="center" justify="between">
                        <Flex align="center" gap="2">
                            <IconPhoto size={24} className="accent" />
                        ...
                        <Dialog.Root open={dialogOpen} onOpenChange={setDialogOpen}>
                            ...
                            <Dialog.Content maxWidth="400px">
                                <Dialog.Title>New Image Project</Dialog.Title>
                    ...
                    <Text size="2" color="gray">Create and manage image generation projects.</Text>
                </Flex>
                    ...
                    <Flex wrap="wrap" gap="4" align="start">
                        {projects.map((project) => (
                            <ProjectCard
                                key={project.id}
                                project={project}
                                onOpen={() => router.push(`/image/${project.id}`)}
